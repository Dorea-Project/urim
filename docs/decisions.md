...
### Q17 — La lecture à voix haute : quelles langues, par quel moyen ? — **tranchée par D60**

🔴 **La réponse n'était pas dans la colonne « comment produire ».** Le fondateur a tranché le 28/08 : les langues locales ne passent pas par un modèle, elles passent par **l'équipe Dorea**. Le tableau ci-dessous garde sa valeur — il dit ce que chaque brique coûte — mais sa deuxième ligne cesse d'être un chantier de recherche pour devenir une file de travail humaine. Voir D60.
...
| Dioula, baoulé, bété… | ~~une **traduction** puis une **voix**~~ — **le cadrage était faux, voir D60** : la voix n'est pas le mur, la traduction l'est ; et ces trois langues ne sont pas à la même distance — le dioula est dans le continuum manding, le baoulé et le bété n'ont aucune route automatique. Quatre routes les remplacent, et la première ne demande rien : **le pasteur s'enregistre lui-même** |
...
### Q18 — Une fois validée, qui voit la synthèse ? — **portée tranchée par D61, destinataire encore ouvert**
...
**D61 tranche ce qui traverse — la synthèse validée, et rien d'autre — et comment : un consentement par pièce, jamais un réglage.** Reste entier ce que Q9 tient : vers *quelle* assemblée, sur la foi de quel annuaire, et qui rattache qui. Tant que Q9 est ouverte, la porte est spécifiée mais ne s'ouvre pas.
...
| D52 | **Q2 tranchée : la transcription tourne sur l'appareil**, avec un modèle embarqué de la famille Whisper | La contradiction était ouverte depuis le début — la file serveur prévoyait `transcrire`, la maquette promettait « sur l'appareil », et les deux ne pouvaient pas être vrais. La spec T-Rec la tranche **par implication sans le dire** : F3 exige un sermon entier sans connexion et I23 que la première détection ne dépende pas du réseau ; sans transcript, pas de détection locale, donc le STT est embarqué ou la spec ne tient pas. **Ce n'est pas une préférence de modèle** : l'étage 2 exige « timestamps mot à mot, score de confiance conservé par mot », I26 pose son seuil par span et I28 ancre la capsule au fragment source. Le reconnaisseur système (`speech_to_text` 7.4.0, déjà au dépôt) rend `recognizedWords` et **une seule confiance pour tout le résultat** — les deux invariants n'auraient rien où s'accrocher ; et sur la plupart des Android il envoie l'audio chez Google, ce qui rendrait D10 faux au sens propre. Vosk tient le hors-ligne mais son français est trop faible pour un transcript qu'on comparera à un squelette ; un service distant casse I23, F3 et D10 d'un coup, et coûte à la minute sur chaque culte de chaque assemblée. **Trois contreparties assumées** : Whisper invente du texte sur les silences — un culte en est plein, il faut une détection d'activité vocale en amont ; il ne sépare pas les locuteurs, donc la réserve de l'écran de relecture reste entière ; et l'accent ivoirien avec passages en dioula fera monter le taux d'erreur — ce que la mesure en trois églises existe précisément pour trouver. Le choix du gabarit — `tiny`, `base`, quantisation — reste au banc d'essai, pas au décret. ⚠️ **Corrigé le 03/09 en le mesurant : la parade nommée ici n'est pas celle qui protège.** Whisper `tiny` invente bien sur trente secondes de silence — *« de la fin de la fin de la fin de la fin… »* — mais le filtre d'activité vocale était **éteint** quand le banc n'a rien tiré de lui. Ce qui jette le texte est en aval, côté décodeur : `no_speech_threshold`, un seul scalaire à 0,6 ; le désarmer seul ramène l'invention, désarmer les deux autres seuils ne change rien. Le mécanisme est contre-intuitif et il faut le comprendre avant de le porter — **le modèle sait qu'il n'entend rien**, il émet une probabilité de « pas de parole » élevée, et son décodeur parle quand même, parce que c'est tout ce qu'un décodeur autorégressif sait faire. La sûreté vient de **lire le drapeau et de jeter le texte**, pas de filtrer l'entrée. La détection d'activité vocale reste une bonne seconde ligne, mais **un portage qui s'appuierait sur elle à la place du seuil n'aurait pas de première ligne** : `whisper.cpp`, ONNX ou autre doit exposer `no_speech_prob`, et le code doit l'appliquer — sinon le pasteur aura des boucles dans son sermon, et personne ne le verra avant un dimanche. Mesuré par `scripts/urim_mesure_transcription.py`, côté back-dorea. |
...
| D58 | **Le texte de la capture arrive par blocs et ne se réécrit jamais** ; ce qui est vraiment en temps réel, c'est le témoin d'écoute | Le fondateur décrit l'expérience voulue : *« son micro est ouvert, il parle, il voit en temps réel le speech to texte »*. La demande est juste, mais elle bute sur un invariant déjà posé. **I24 gouverne l'affichage autant que le transport** : une fenêtre glissante fait redécoder à Whisper de l'audio déjà affiché, donc **réécrit des mots que le pasteur vient de lire**. Un texte qui se corrige sous les yeux est pire qu'un texte qui tarde — on ne sait plus lequel croire, et l'additivité stricte cesse d'être vraie à l'écran alors qu'elle l'est partout ailleurs. Le texte s'ajoute donc **par fragment, une fois pour toutes**. ⚠️ **La voie du vrai mot à mot a été regardée et refusée** : le reconnaisseur système la donne — il est déjà au dépôt — mais sur la plupart des Android il envoie l'audio chez Google, ce qui rendrait D10 faux au sens propre, et il ne porte ni horodatage par mot ni confiance par mot (D52). On ne paiera pas la promesse pour un effet d'affichage. **Ce qui reste réellement instantané, et qui suffit** : le niveau du micro, la durée qui court, le compteur de fragments écrits. La fonction que le temps réel remplit ici n'est pas de lire — personne ne lit son téléphone en chaire — c'est de **savoir que la machine entend**, et ces trois signaux la remplissent sans rien promettre de faux. Le pas d'affichage descend sous trente secondes si le banc d'essai du sprint 8 dit que l'A07 tient les deux modèles à la fois ; **il ne devient jamais réécrivable**. |
| D59 | **La synthèse arrive par la préparation, pas par le transcript** — les trois onglets sortent sans ouvrir le verrou | Le fondateur veut trois onglets : le texte brut, la synthèse détaillée de Mistral à valider, le résumé avec l'audio. Le deuxième heurte de plein fouet le verrou du serveur : les étapes 2 à 4 n'ouvrent pas avant la mesure du taux d'erreur en trois églises, et `capture/domain.py` l'écrit en code, pas en commentaire. **Le contourner serait ouvrir la synthèse sur un transcript dont personne n'a mesuré la qualité** — exactement ce que la mesure existe pour empêcher, et sur de la matière scripturaire. Mais le verrou porte sur **le chemin « transcript → synthèse »**, pas sur la synthèse. Or une prédication préparée dans Urim porte déjà son texte : péricope bornée, axe retenu, plan rédigé, articulations — **du texte passé par le moteur, donc déjà conforme à I25**. La synthèse de celle-là ne demande aucun transcript et n'ouvre aucune étape. Donc : **les trois onglets sortent maintenant**, et le deuxième se remplit pour toute prédication rattachée à une préparation. Pour une capture sans préparation — un pasteur qui appuie sur le micro sans être passé par Urim — l'onglet dit ce qu'il attend et pourquoi (D13), et c'est la mesure qui le débloquera. **Effet de bord assumé, et il est voulu** : préparer dans Urim donne visiblement plus que capter seul, ce qui est la vérité du produit. |
| D60 | **Q17 tranchée : une langue locale se gère par une voix humaine, et par quatre routes de coût très différent** — la traduction automatique n'en est pas une | Q17 posait le problème à l'envers, et le code aussi : `ReadAloudKind.translated` traite le dioula, le baoulé et le bété comme un seul cas, « traduire puis dire ». **Deux choses sont fausses là-dedans.** ⚠️ **La voix n'est pas le mur, la traduction l'est.** On suppose qu'il manque une voix de synthèse ; c'est l'inverse — les modèles multilingues massifs de synthèse vocale sont entraînés en grande partie sur des **lectures d'Écriture**, donc exactement sur ce registre, et toute langue qui a un Nouveau Testament audio est plausiblement couverte. Ce qui manque, c'est **le texte juste à lire**. ⚠️ **Et ces langues ne sont pas à la même distance.** Le dioula appartient au continuum manding — mutuellement intelligible avec le bambara — qui est parmi les mieux dotés d'Afrique de l'Ouest et présent dans les grands modèles de traduction ; **le baoulé (kwa) et le bété (kru) n'y sont pas**. Leur donner la même phrase à l'écran est un mensonge par uniformité. 🔴 **Mais même là où la route existe, on ne la prend pas** : une phrase doctrinale traduite par une machine, lue à une assemblée, sans que personne ne puisse la vérifier — le commentaire du code le dit déjà, *« ce n'est pas un défaut d'ergonomie, c'est une faute »*. **Donc quatre routes, dans cet ordre.** **(1) Le pasteur s'enregistre lui-même** : il parle la langue de son assemblée, trois minutes de résumé en baoulé dans sa propre voix ne demandent **aucune technologie nouvelle** — le micro et la permission sont déjà là, et `ownVoice` existe déjà dans le domaine. ⚠️ *Corrigé le 28/08 en le construisant : ce n'est pas `SermonCapture` qu'on réutilise. Une capture de culte est du PCM brut en fragments, purgé au septième jour, taillé pour Whisper ; une piste est un fichier compressé qui se joue et qui vit avec sa synthèse. Le greffon est commun, la brique non.* C'est la meilleure des quatre, pas la moins bonne : c'est la voix que l'assemblée reconnaît. **(2) L'interprète de l'assemblée existe déjà** — dans une église ivoirienne, quelqu'un interprète phrase par phrase depuis l'estrade. La question n'est donc pas « comment traduire », c'est **« que donne-t-on à celui qui fait déjà ce travail »** : la synthèse **avant** dimanche, et un lexique des noms propres et des termes doctrinaux qui s'accumule par église — c'est précisément là que les interprètes butent. Aucun modèle, livrable tout de suite. **(3) L'équipe Dorea**, pour les assemblées qui n'ont pas d'interprète : une file de travail avec un état (soumise, prise, rendue, refusée), une langue, un délai annoncé. **(4) La machine, jamais seule** — au mieux un brouillon posé devant un locuteur, qui signe ou refuse ; jamais une sortie. **Ce qui se range en base n'est donc pas « une traduction », c'est une piste** : sa langue, sa **provenance** — voix du pasteur, interprète, équipe Dorea, machine relue — son état, et ce qui revient. ⚠️ **La provenance se lit toujours**, comme `signedByMachine` le fait déjà ailleurs : une assemblée doit savoir si elle entend son pasteur, un interprète ou un modèle. Et **une demande n'est jamais perdue en silence** — *« une promesse qui échoue en silence est pire que pas de promesse »*. |
| D61 | **La sortie vers la plateforme Dorea est un geste consenti, une fois par pièce, et ne porte que la synthèse validée** | *« Il peut soumettre sur la plateforme Dorea app »* traverse la frontière que le profil promet aujourd'hui : *« Ce que vous écrivez dans Urim ne traverse jamais vers elles »*. Q18 l'avait vu venir — « une promesse inverse de celle du profil » — et la contradiction est réelle, pas rhétorique. **Elle se résout par la portée, pas en réécrivant la promesse.** Ce qui traverse est **la synthèse validée, et elle seule** : ni la préparation, ni ses tours, ni le transcript, ni l'audio. La promesse d'étanchéité reste donc vraie mot pour mot pour tout ce qu'elle couvrait, et la synthèse devient le seul objet du système qui ait une sortie — parce que c'est le seul que le pasteur ait explicitement relu et validé pour être entendu. **Le consentement est par pièce, jamais un réglage** : un interrupteur « publier mes synthèses » deviendrait un oubli, et l'oubli publierait. Le texte du profil change **en même temps** que la route s'ouvre — laisser la promesse absolue pendant qu'une porte existe serait mentir par omission. ⚠️ **Ce qui reste ouvert et bloque la livraison** : Q9 — d'où vient l'annuaire des églises, qui rattache qui — parce qu'on ne publie pas vers une assemblée dont le rattachement n'est pas défini. |
| D62 | **L'interprétation en langue locale est un service tenu par l'équipe Dorea** — un interprète maîtrisé par langue, et la voix du pasteur clonable sur celle de l'interprète, s'il le demande | ⚠️ **Cette décision remplace celle du 28/08 au matin**, qui disait « on ne traduit pas, on fait dire » et confiait la piste au pasteur lui-même. Les trois arguments du fondateur la renversent, et deux d'entre eux répondent précisément aux objections qui la fondaient. **(1) Un seul interprète maîtrisé par langue vaut mieux que cinquante pasteurs qui font de leur mieux.** L'argument « c'est la voix que l'assemblée reconnaît » masquait un défaut réel : chaque pasteur aurait rendu son propre baoulé, le vocabulaire doctrinal aurait dérivé d'une église à l'autre, et **rien n'aurait pu être amélioré** — aucune boucle de qualité. Centraliser rend le lexique **commun au lieu de per-église**, et rend la qualité mesurable donc perfectible. **(2) Le clonage de la voix dissout l'objection.** Ce qui plaidait pour « le pasteur s'enregistre » était la reconnaissance de sa voix ; le clonage donne **sa voix et les mots de l'interprète** — strictement mieux que l'un ou l'autre seul. **(3) Le service est finançable**, ce qui lève l'objection qui bloquait cette route : « ni personnes ni délai à annoncer ». Une équipe payée est une équipe qui existe, et les paires produites nourrissent le modèle. 🔴 **Trois gardes non négociables sur le clonage, et elles ne sont pas de la prudence décorative.** Faire dire à un pasteur, dans sa voix, des mots qu'il n'a pas prononcés est une parole synthétique attribuée à un homme réel — devant une assemblée qui le croit. Donc : **le consentement est explicite et par pièce**, jamais un réglage (même forme que D61) ; **le pasteur écoute avant diffusion et peut refuser** — une piste non écoutée ne sort pas ; **la provenance se lit toujours**, comme `signedByMachine` ailleurs : l'assemblée doit savoir qu'elle entend une voix reconstituée sur une interprétation. Et l'empreinte vocale **disparaît avec le compte**, comme le reste (D47). ⚠️ **Ce que ça change dans l'usage, et qu'il faut dire** : une interprétation prend des jours, pas des minutes. **Ce n'est pas une fonction du dimanche matin** — c'est une diffusion après coup, que l'assemblée écoute dans la semaine. La piste arrive quand elle arrive, et l'écran doit l'annoncer ainsi plutôt que de laisser espérer autre chose. **Ce qui reste vrai de la version précédente** : la langue est **nommée** par le pasteur, jamais choisie dans un menu fermé ; l'interprétation porte sur la **synthèse validée**, jamais sur le transcript brut ; et une demande n'est jamais perdue en silence — état, langue, délai annoncé. **Le pasteur qui s'enregistre lui-même reste possible** : c'est la sortie immédiate et hors réseau, pour qui ne veut pas attendre. |
| D63 | **Le malinké est la langue par défaut — proposée, jamais imposée** — et les interprétations produites alimentent un moteur d'apprentissage, **sous un consentement distinct de celui de la diffusion** | Les langues sont sur demande, mais Dorea en maîtrise une aujourd'hui, et c'est celle qu'on propose. **Le choix est techniquement le bon, pas seulement le disponible** : le malinké appartient au continuum manding — mutuellement intelligible avec le bambara et le dioula — qui est le groupe le mieux doté d'Afrique de l'Ouest. Ce qu'on apprend sur le malinké **transfère** vers le dioula et le bambara ; l'apprendre sur le baoulé (kwa) ou le bété (kru) ne transférerait à rien. 🔴 **Et la concentration est la condition du moteur, pas une restriction d'offre** : disperser les interprétations sur dix langues produirait dix jeux trop maigres pour entraîner quoi que ce soit ; les concentrer sur une seule produit **un corpus**. C'est la raison pour laquelle « une langue par défaut » n'est pas un pis-aller. **La proposition doit dire pourquoi elle est faite.** Le produit a déjà ce motif deux fois — D50 justifie la page d'ouverture à l'écran, D55 fait dire à Urim pourquoi il a tranché — et la règle est la même : *un automatisme qui ne se justifie pas passe pour une panne*. ⚠️ **Ici le prix d'une phrase mal tournée est plus haut qu'ailleurs, parce qu'une langue est une identité.** Un pasteur baoulé à qui on propose le malinké sans motif lira une présomption sur qui il est. L'écran dit donc **ce que Dorea sait faire aujourd'hui**, jamais ce que le pasteur est censé parler — et le refus se fait en un geste, sans justification demandée. 🔴 **La collision qu'il faut regarder en face : l'application promet le contraire par écrit.** `privacyNoResaleBody` dit *« aucun entraînement de modèle sur votre contenu »* et `settingsTrainingNotice` dit *« Urim n'utilise jamais vos préparations pour entraîner un modèle »*. Entraîner sur les paires sans y toucher serait un mensonge, et le genre de mensonge qui ne se rattrape pas. **La résolution passe par la portée, comme D61.** La promesse reste **vraie et inchangée pour les préparations** — c'est la partie intime, elle ne bouge pas. Ce qui peut nourrir le moteur est la **synthèse validée**, seul objet du système qui ait déjà une vie au-dehors, **et son interprétation**. ⚠️ **Deux consentements séparés, jamais un seul** : accepter d'être entendu n'est pas accepter d'entraîner. Deux cases sur le même écran, chacune refusable sans perdre l'autre. Et le texte des réglages est **corrigé en même temps** que la collecte commence. **Ce que la paire doit porter pour valoir quelque chose** : segmentée et alignée — synthèse et interprétation appariées morceau par morceau, pas deux blocs (même principe que I28) ; **l'interprète identifié**, pour pouvoir retirer plus tard tout ce qu'un mauvais relecteur a produit ; et l'état d'acceptation du pasteur, **un refus étant un signal, pas un silence**. **La trajectoire vers l'automatisation** : l'homme écrit, puis le modèle propose et l'homme relit, puis l'homme sonde. ⚠️ **Le passage d'un étage au suivant se mesure, il ne se décrète pas** — même culture que le gabarit de Whisper au banc (D52) et que le double seuil de la mesure T-Rec. Et **la machine ne remplace jamais le relecteur, elle remplace la page blanche**. |
| D64 | **L'identifiant de capture devient un UUIDv4, produit par l'appareil** | Le fichier portait depuis le début la phrase qui décrivait sa propre échéance : *« suffisant tant que les identifiants sont produits et consommés localement ; le jour où deux appareils créeront des entités hors ligne avant de se synchroniser, passer à un UUID v4 — ce fichier est le seul à changer »*. **La capture est exactement ce cas** : l'appareil ouvre une capture sans demander au serveur, parce qu'un dimanche ne se rejoue pas et qu'aucun appel d'ouverture ne doit pouvoir échouer sans réseau (D53). L'ancienne forme — `'$stamp-$salt'` en base 36, « m3x8q1k-0a2bf9z » — se heurtait à `Capture.id: UUID`, et la route de dépôt ne pouvait pas la lire comme clé primaire. 🔴 **Le moment du changement est tout l'enjeu** : des fragments montés sous un identifiant ne se renomment plus sans perdre le lien, donc il fallait le faire **avant la première captation réelle**, où il ne coûte rien — après, il coûterait un dimanche. **Deux options ont été écartées** : une colonne d'appareil à côté de la clé serveur laissait deux identités pour un même objet et une jointure sur le chemin le plus chaud ; un UUIDv5 dérivé de la chaîne évitait tout changement client mais rendait la source irrécupérable. ⚠️ **Et le tirage devient cryptographique** : dix appareils qui démarrent le même dimanche matin tirent leur graine du même instant, et un générateur amorcé par l'horloge leur donnerait des suites voisines — une collision ferait écrire deux cultes l'un sur l'autre. Un test tient la forme, l'unicité sur deux cent mille tirages, et **l'absence de `_`** : `CaptureStore` nomme ses dossiers `<id>_<millis>` et les recoupe sur ce caractère ; un identifiant qui le contiendrait rendrait la capture invisible, donc perdue sans le moindre message. |
| D65 | **Les octets audio de la campagne vivent dans un stockage objet**, pas sur le disque du serveur | Un culte pèse 77 Mo (D53) ; dix pasteurs sur quatre dimanches font 3 Go, pour de l'audio qui vit sept jours. ⚠️ **Le dépôt range pourtant déjà ses fichiers sur disque local** — `media_dir = "media_uploads"`, commenté « dev » — et aucune dépendance de stockage objet n'existe : ce choix **ajoute une brique** pour un usage que D56 veut voir s'éteindre quand le verdict de la mesure tombe. Le prix est assumé : la durabilité et l'absence de volume à gérer valent la dépendance, et un conteneur sans volume persistant perdrait l'audio d'une campagne qu'on ne rejouera pas. 🔴 **La borne de D56 ne bouge pas pour autant** : la route reste explicitement activable et se coupe au verdict — un stockage plus commode ne doit pas devenir la raison de le garder par inertie. |
| D66 | **La synthèse née d'une préparation vit dans sa propre table**, à côté du Retour et jamais dedans | `urim_reflection.capture_id` est unique et non nul — *un Retour par culte* — et la contrainte `synthese_validee_signee` refuse déjà une synthèse `validee` sans signataire. Or D59 fait naître une synthèse de la **préparation**, sans transcript et sans capture : elle n'a pas de `capture_id` à donner. **Rendre la colonne nullable a été écarté**, et pour une raison de fond plutôt que de forme : cette contrainte existait précisément pour dire « un Retour par culte », et la relâcher rendrait fausse la phrase qu'elle portait — le commentaire du modèle l'affirme encore. Le prix assumé est **deux chemins vers un objet métier voisin** ; le gain est que chacune des deux tables continue de dire la vérité sur ce qu'elle contient. ⚠️ **Ce qui ne change pas** : rien ne sort d'une synthèse non validée, la validation pose `validated_by` **et** `validated_at`, et la proposition se lit séparément de ce que le pasteur a retenu — même raison que `theme` contre `theme_label`. |
| D67 | **L'application est verrouillée en portrait** | Le tiroir de l'accueil débordait de 177 points en paysage : ses entrées fixes — en-tête, deux actions, séparateur, étiquette, quatre lignes de pied — dépassent à elles seules les 354 points que laisse un écran couché sur un Galaxy A07. **Réglages et Profil devenaient inatteignables**, et rien ne le disait. Deux remèdes se valaient techniquement : rendre le tiroir défilant, ou refuser le paysage. **Le second dit la vérité sur l'usage** — un pasteur tient son téléphone debout, en chaire — et il couvre **aussi les écrans qu'on n'a pas encore vus tourner**, alors que le premier n'aurait corrigé que celui qu'on avait trouvé. ⚠️ *Une mise en page qu'on ne teste jamais finit par mentir.* Le verrou est posé en Dart plutôt qu'au manifeste, pour rester lisible et testable au même endroit que le reste. **À rouvrir le jour où une tablette entre dans le décor** : ce jour-là ce n'est pas cette ligne qu'il faudra retirer, c'est chaque écran qu'il faudra regarder couché. |
| D68 | **L'assemblée voyage avec la capture, écrite à l'arrêt du micro — jamais déduite du profil** | Le pilote compte **dix pasteurs pour sept églises** : la supposition « un pasteur, une assemblée » ne tient pas. Un pasteur qui en dessert deux prêche dans **l'une** d'elles ce dimanche-là ; une valeur par défaut lue dans son profil serait donc fausse une fois sur deux, **et fausse en silence**. 🔴 **Le coût d'une erreur n'est pas cosmétique** : le double seuil de la mesure T-Rec compte quinze avis venus de **trois églises distinctes**, et un culte rangé sous la mauvaise assemblée le fausserait sans que personne ne le voie. **La première version du transport résolvait l'église au départ** et refusait d'envoyer dès que le pasteur en avait zéro ou plusieurs — juste sur le principe, mais l'envoi restait bloqué à jamais et rien à l'écran ne le disait. L'église est donc désormais un **témoin sur le disque**, `eglise`, à côté de la marque haute et du témoin d'arrêt — même patron que `CaptureFormat.endMarker`, même raison : *le disque fait foi*. ⚠️ **Elle s'écrit à l'arrêt, jamais au démarrage** — *rien ne s'interpose* entre le pasteur et son micro. Une seule église : on l'attache sans rien demander, car poser une question dont la réponse est évidente est un geste pour rien. Plusieurs ou aucune : **la capture attend**, avec ses sept jours devant elle, et `sansEglise()` la rend listable pour que l'écran puisse demander. Une capture ne change jamais d'assemblée après coup : réécrire ce témoin ferait diverger ce que le serveur a rangé de ce que l'appareil croit. |
| D69 | **Le gabarit ne se choisit pas sur le seul taux d'erreur** — l'invisibilité de la panne monte avec la qualité du modèle | Le banc des 02 et 03/09 a donné le même silence de trente secondes à deux décodeurs autorégressifs, et ils n'ont pas échoué de la même façon. Whisper `tiny` bégaie — *« de la fin de la fin de la fin de la fin… »* — une boucle dégénérée qu'un relecteur repère en une seconde. `gemini-3.6-flash`, sur le même vide, écrit de la prose impeccable : une stratégie numérique pour le Bénin, la consigne de l'épreuve orale du DELF A2, une charte de marque sur le devoir de ne pas induire le consommateur en erreur. Sept passes, sept textes différents, **aucun distinguable d'une vraie transcription**. Les deux inventent ; un seul se trahit. 🔴 **La conséquence gouverne le banc d'essai que D52 laisse ouvert** : on ne peut pas mesurer la sûreté sur un petit gabarit et l'extrapoler au grand. Le jour où l'on passera de `tiny` à `base` ou `small` pour baisser le taux d'erreur, **on achètera de la qualité en payant en invisibilité** — et le gain se lira dans la mesure là où la perte ne s'y lira pas, ce qui est exactement la façon dont une décision se prend à l'envers en toute bonne foi. Le banc rend donc **deux chiffres par gabarit, jamais un** : le taux d'erreur sur la parole, et ce qui sort sur le silence. Un gabarit qui gagne sur le premier et perd sur le second n'est pas un meilleur gabarit. ⚠️ **Et le garde-fou ne se transporte pas d'un modèle à l'autre** : `no_speech_threshold` sauve Whisper parce que Whisper bégaie ; il ne sauverait pas un modèle dont l'invention est bien formée. |
| D70 | 🔴 **Le tronc de la branche ② est l'audio retravaillé, pas le texte** — enregistrement, édition, pièce, publication ; la transcription, la synthèse et l'interprétation sont trois branches optionnelles | Un pasteur a décrit son dimanche réel, et ce n'est pas celui que le produit portait. Il enregistre **1 h 30 d'un seul tenant** — une heure de prédication enchaînée par trente minutes de prière. Il réécoute, il avance, il met en pause. Il constate du bruit, des chants au démarrage de la prière. **Il ne veut ni transcrire, ni publier, ni faire interpréter** : il veut *éditer son audio, le réduire, le découper en deux parties séparées*. Puis publier la prière le mardi ; le vendredi la prédication, avec son texte ; le samedi demander la version en langue, la recevoir, et la publier le samedi soir. **Le produit n'est pas le transcript, c'est l'audio** — et le texte est une option, sur une pièce, le vendredi. 🔴 **Ce que ça fait sauter :** le verrou de la mesure en trois églises avait été posé sur un tronc où *tout* passait par le transcript ; il ne change pas de sévérité mais de **portée**, et ne garde plus que la flèche `transcription → synthèse`. L'édition, la publication de l'audio et l'interprétation ne l'ont jamais traversé — elles étaient bloquées par un verrou qui ne les concernait pas. C'est la même erreur que D59 a déjà corrigée une fois : *la synthèse attendait un transcript dont elle n'avait pas besoin*. ⚠️ **L'interprétation part de l'audio écouté, pas du texte** : l'équipe Dorea entend la pièce, donc elle ne dépend ni de Whisper, ni de `decider()`. Elle est livrable aujourd'hui — et le corpus de D63 change de paire au passage, *audio ↔ interprétation* et non plus *synthèse validée ↔ interprétation*. **Ce que ça crée :** *la pièce* comme objet de premier rang — ses bornes dans la matière, son titre, sa date de sortie — et l'éditeur qui la produit ; se déplacer dans 1 h 30, poser deux points de coupe, réécouter les bornes. `capture_playback` ne sait aujourd'hui ni chercher, ni mettre en pause, ni donner une position : sur une heure et demie, une lecture linéaire pour retrouver la fin de la prédication prendrait une heure. **Couper, en revanche, est la partie la moins chère de tout le chantier** : le PCM est à débit constant, 32 000 octets valent une seconde, et une coupe à la 62ᵉ minute est un décalage de 119 040 000 octets — pas de décodage, pas de bibliothèque. ⚠️ **Ce que ça ne change pas : un micro capte la salle.** La matière brute reste purgée au septième jour — c'est ce qui a été pris *sans intention*, et un témoignage donné au micro du prédicateur peut s'y trouver. Seule la pièce survit, comme une piste vit avec sa synthèse (`voice_track_store`). 🔴 **Le découpage est le consentement** : le pasteur a écouté avant de couper, donc ce qu'il garde est ce qu'il a décidé de garder ; c'est cet acte, et non une durée de rétention, qui transforme une matière captée en pièce assumée. Les sept jours deviennent donc un **délai de découpage**, pas un délai de publication — et ils s'annoncent, ils ne surprennent pas. **Trois conséquences à tenir dans le code** : D61 ne porte plus « que la synthèse validée » mais l'audio retravaillé ; `urim_reflection.capture_id` unique et non nul interdit qu'un dimanche donne deux pièces ; et le choix PCM de D53 se rouvre le jour où publier exigera autre chose que de la qualité dictaphone — pas avant, et pas sans l'avoir mesuré. ✅ **Tranché le même jour** : il y a bien **deux synthèses, distinguées par leur origine** — celle de D59 naît *de la préparation*, avant le culte, c'est ce qu'il a prévu de dire ; celle-ci naît *du transcript de la pièce*, c'est ce qu'il a réellement dit, et c'est elle qui sort en epub. |
| D71 | ⛔ **On ne transcrit jamais la matière brute — seule une pièce se transcrit** ; et le pasteur voit **le transcript ET la synthèse**, jamais la synthèse seule | Deux règles qui n'en font qu'une : *aucun modèle ne lit ce que le micro a pris sans intention, et aucun texte fabriqué ne s'affiche sans ce dont il a été tiré*. **La première prolonge D70 jusqu'au bout.** Le découpage est le consentement ; transcrire le brut contournerait ce consentement par la porte de derrière — un témoignage donné au micro du prédicateur pendant trente minutes de prière n'a pas à entrer dans un transcript parce qu'il se trouvait dans la salle. ⚠️ **Le port change donc de sujet** : `Transcriber.transcribe(chunks, language_hints)` prend aujourd'hui les fragments d'une capture, il prendra **une pièce**. Ce n'est pas un détail de signature — c'est ce qui fait tenir la règle par le code plutôt que par la discipline. **Deux conséquences en cascade** : la mesure des trois églises porte dès lors sur des pièces, ce qui est meilleur — l'audio y est propre, chants et bruit écartés, et le relecteur juge un transcript de ce que le pasteur a *assumé* ; et si rien ne transcrit les cent quatre-vingts fragments bruts, **la dernière raison de les faire monter au serveur tombe**. 🔴 **La seconde règle est une parade, pas de la transparence de confort — et c'est la seule du produit.** Le pasteur sait ce qu'il a prêché. Une invention dans le transcript, il la repère à l'œil : la boucle « de la fin de la fin de la fin » de Whisper `tiny`, ou le paragraphe sur la stratégie numérique du Bénin que `gemini-3.6-flash` a produit sur trente secondes de silence (D69). **La synthèse seule la cacherait** — elle serait fluide, plausible, et bâtie sur du faux. C'est l'endroit exact, et le seul, où un humain peut attraper une invention avant qu'elle ne devienne un souvenir. Et ça donne sa raison d'être à ce que D58 impose déjà : un transcript qui s'ajoute par blocs et ne se réécrit jamais est un transcript qu'on peut relire contre sa synthèse. Pendant serveur : D25 et D26 (S6 §1bis). |
| D72 | ⛔ **La préparation ne porte ni transcription ni synthèse — Prêcher les tient toutes les deux**, et la synthèse fabriquée depuis le plan **s'éteint** | 🔴 **Ceci renverse D59, et D59 avait moins d'un mois.** D59 faisait naître la synthèse *de la préparation* — péricope, axe, plan écrit — précisément parce que le chemin du transcript était verrouillé : *« elle sort aujourd'hui »* au lieu d'*« après des mois de campagne »*. C'était un pont, et il a servi. Le tronc ayant changé (D70), la synthèse rentre chez elle. **Ce qu'elle résumait n'était pas un sermon.** Une synthèse bâtie sur le plan résume **une intention**, pas une prédication — et si elle est publiée, lue à voix haute devant une assemblée, ou interprétée en malinké, elle présente un projet comme si c'était la parole prononcée. C'est la même famille d'erreur que l'invention du modèle : quelque chose de plausible qui tient la place de quelque chose de réel. La règle du dépôt tranche donc dans le même sens qu'ailleurs — ***rien plutôt qu'une vraisemblance***. ⚠️ **Ce qui s'éteint, nommément, et c'est du code livré** : la fabrique de synthèse depuis une étude (`GET /studies/{id}/synthese`, `POST /studies/{id}/synthese/validation`, `synthese_service.py` et son dépôt), et avec elle la raison d'être de D66 — *la synthèse née d'une préparation vit dans sa propre table*. La table reste, sa clé change : elle pend d'une **pièce**, plus d'une étude. **Ce qui ne s'éteint pas mais change d'attache** : la validation par signature, la lecture à voix haute (`aloud_reader`) et la piste de voix du pasteur (`voice_track_*`) — les outils restent entiers, ils lisent désormais la synthèse d'une pièce. Côté application, **les écrans n'ont pas à bouger** : `synthesis_page.dart`, `capture_shell.dart`, `sermon_shell.dart`, `aloud_view_model.dart` et `voice_track_view_model.dart` vivent déjà sous `presentation/transcription/`. Ce qui était mal placé n'était pas l'écran, c'était la source. 🔴 **Le prix est réel et il est assumé** : la synthèse naît du transcript, le transcript attend la mesure des quinze avis dans trois églises, donc **la synthèse, sa signature, sa lecture à voix haute et sa piste attendent aussi**. La phrase du cahier — *« la branche ① est complète, un pasteur va de sa première phrase à une piste audio dans la langue de son assemblée »* — **devient fausse le jour où ceci est appliqué**. ⚠️ **Et ceci met D22 de côté délibérément** — *on ne démolit pas avant d'avoir remplacé*. La règle tient partout ailleurs ; ici le remplacement est verrouillé pour des mois, et garder l'ancien pendant ce temps reviendrait à laisser un résumé d'intention passer pour un résumé de sermon aussi longtemps que dure la mesure. Le fondateur a tranché en connaissant ce prix. Pendant serveur : D27 (S6 §1bis). |
...
| ~~Le document produit est **posé dans un dossier**, jamais partagé : `share_plus` exige `win32 ^5` et le coffre à secrets `^4`~~ ✅ **Levé le 06/09** — le coffre est passé en `^11` et le conflit n'existe plus. Vérifié en le résolvant, pas en relisant cette ligne. `share_plus` est au dépôt, et les **pièces** s'envoient déjà (`FileSharer`). ⚠️ **Reste dû pour le document** : la fiche de chaire et les diapositives ne sont toujours pas partagées — le mur est tombé, le bouton n'est pas posé | Un geste sur le livrable, désormais sans obstacle |
